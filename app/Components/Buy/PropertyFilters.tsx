...
import { PropertyFilters as PropFilters } from "@/app/lib/properties";
import {
  Search,
  ChevronDown,
  MapPin,
...
export default function PropertyFilters({
  ...
  const updateFilters = (newValues: Partial<PropFilters>) => {
          ...
          <div className="w-full lg:w-auto flex flex-wrap lg:flex-nowrap items-center gap-4">
            ...
            <div className="flex-1 lg:w-52 group relative">
              ...
              <select
                ...
                className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-12 pr-10 text-white text-sm appearance-none focus:outline-none focus:border-gold-400/50 transition-all cursor-pointer"
              >
                <option value="" className="bg-navy-900 text-white">
                  Property Type
                </option>
                <option value="apartment" className="bg-navy-900 text-white">
                  Apartment
                </option>
                <option value="villa" className="bg-navy-900 text-white">
                  Villa
                </option>
                <option value="townhouse" className="bg-navy-900 text-white">
                  Townhouse
                </option>
            ...
            <div className="flex-1 lg:w-40 group relative">
              ...
              <select
                ...
                onChange={(e) =>
                  updateFilters({
                    bedroom: e.target.value ? Number(e.target.value) : "",
                  })
                }
                className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-12 pr-10 text-white text-sm appearance-none focus:outline-none focus:border-gold-400/50 transition-all cursor-pointer"
              >
                ...
                <option value="2" className="bg-navy-900 text-white">
                  2 Bedrooms
                </option>
                <option value="3" className="bg-navy-900 text-white">
                  3 Bedrooms
                </option>
                <option value="4" className="bg-navy-900 text-white">
                  4+ Bedrooms
                </option>
              </select>
              <div className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500 pointer-events-none group-hover:text-gold-400 transition-colors">
            ...
            <div className="flex-1 lg:w-44 group relative hidden">
              ...
              <select
                onChange={(e) => {
                  ...
                  updateFilters({
                    minPrice: Number(min.replace("M", "")) * 1000000,
                    maxPrice: Number(max.replace("M", "")) * 1000000,
                  });
                }}
                className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-12 pr-10 text-white text-sm appearance-none focus:outline-none focus:border-gold-400/50 transition-all cursor-pointer"
              >
                ...
                <option value="0-1M" className="bg-navy-900 text-white">
                  Under $1M
                </option>
                <option value="1M-3M" className="bg-navy-900 text-white">
                  $1M - $3M
                </option>
                <option value="3M-5M" className="bg-navy-900 text-white">
                  $3M - $5M
                </option>
                <option value="5M+" className="bg-navy-900 text-white">
                  $5M+
                </option>
              </select>
              <div className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500 pointer-events-none group-hover:text-gold-400 transition-colors">
            ...
            {/* Search Button */}
            <button className="hidden w-full lg:w-auto px-10 py-4 bg-gold-400 hover:bg-gold-500 text-navy-950 font-bold rounded-2xl transition-all shadow-lg active:scale-95 cursor-pointer">
              Find Property
            </button>
            <button
